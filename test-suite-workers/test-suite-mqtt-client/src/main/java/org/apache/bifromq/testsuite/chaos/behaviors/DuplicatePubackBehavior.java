...
public class DuplicatePubackBehavior extends AbstractChaosBehavior {

    @Override
    public String name() {
        return "DUPLICATE_PUBACK";
    }

    @Override
    protected CompletableFuture<Void> sendViolation(ChaosContext ctx) {
        int packetId = ctx.startPacketId();
        byte[] puback = MqttFrameEncoder.puback(packetId);
        
        return ctx.connection().sendRaw(puback)
            .thenCompose(v -> ctx.connection().sendRaw(puback));
    }
}
