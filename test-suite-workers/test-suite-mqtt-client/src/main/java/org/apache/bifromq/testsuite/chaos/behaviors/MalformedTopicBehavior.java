...
public class MalformedTopicBehavior extends AbstractChaosBehavior {

    private static final byte[] PAYLOAD = "chaos-malformed-topic".getBytes();
    ...
    protected CompletableFuture<Void> sendViolation(ChaosContext ctx) {
        ...
        String malformedTopic = ctx.topic() + "/#/chaos";
        int packetId = ctx.startPacketId();
        byte[] frame = MqttFrameEncoder.publishQos1(malformedTopic, PAYLOAD, packetId);
