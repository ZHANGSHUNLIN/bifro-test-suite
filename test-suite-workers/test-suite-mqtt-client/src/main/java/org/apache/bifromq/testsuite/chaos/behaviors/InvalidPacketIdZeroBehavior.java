...
public class InvalidPacketIdZeroBehavior extends AbstractChaosBehavior {

    private static final byte[] PAYLOAD = "chaos-invalid-pid".getBytes();

    @Override
    public String name() {
        return "INVALID_PACKET_ID_ZERO";
    }

    @Override
    protected CompletableFuture<Void> sendViolation(ChaosContext ctx) {
        byte[] frame = MqttFrameEncoder.publishQos1(ctx.topic(), PAYLOAD, 0 /* invalid packetId=0 */);
