...
public class DoubleConnectBehavior extends AbstractChaosBehavior {

    @Override
    public String name() {
        return "DOUBLE_CONNECT";
    }

    @Override
    protected CompletableFuture<Void> sendViolation(ChaosContext ctx) {

        byte[] connectFrame = MqttFrameEncoder.connect("chaos-double-connect", 60, true);
        return ctx.connection().sendRaw(connectFrame);
    }
}
