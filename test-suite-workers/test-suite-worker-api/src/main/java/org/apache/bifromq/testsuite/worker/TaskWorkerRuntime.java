...
public interface TaskWorkerRuntime {

    TaskWorker create(Vertx vertx, WorkerTaskCommand command);

    Map<String, TaskStage> runningTaskStages(Map<String, TaskWorker> runningTaskMap);

    ClientQueryResponse queryClients(ClientQueryRequest request, TaskWorker taskWorker);

    NodeMetricsResponse queryMetrics(NodeMetricsRequest request);

    TaskMetricsCleanupResponse cleanupMetrics(TaskMetricsCleanupRequest request);
}
