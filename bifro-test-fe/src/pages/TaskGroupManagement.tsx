...
import type { TaskGroupListItem } from '../types/taskGroup';
import dayjs from 'dayjs';

const { Title } = Typography;

const TaskGroupManagement: React.FC = () => {
    const [groups, setGroups] = useState<TaskGroupListItem[]>([]);
    const [loading, setLoading] = useState(false);
    const [isModalVisible, setIsModalVisible] = useState(false);
    const [editingGroup, setEditingGroup] = useState<TaskGroupListItem | null>(null);
    ...
    const loadGroups = async () => {
        setLoading(true);
        try {
            const pageInfo = await taskGroupApi.getAllGroups();
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'action',
            width: 180,
            render: (_: unknown, record: TaskGroupListItem) => (
    ...
    const handleEdit = (group: TaskGroupListItem) => {
    ...
    const handleDelete = async (id: string) => {
        try {
            await taskGroupApi.deleteGroup(id);
    ...
    const handleModalOk = async () => {
        try {
            ...
            if (editingGroup) {
                await taskGroupApi.updateGroup(editingGroup.id, request);
                message.success('更新成功');
            } else {
                await taskGroupApi.addGroup(request);
