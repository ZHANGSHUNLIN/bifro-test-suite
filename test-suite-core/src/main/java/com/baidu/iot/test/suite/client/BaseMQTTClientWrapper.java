...
public abstract class BaseMQTTClientWrapper implements MQTTClientWrapper {
    ...
    /**
     * 检查连接前的任务状态
     * 允许连接的状态：初始化阶段(连接客户端)、进行中、收集结果
     * 不允许连接的状态：正在关闭、已关闭、已停止、中断中
     */
    protected boolean canConnect(TaskStage stage) {
        if (stage == TaskStage.SHUTDOWN || stage == TaskStage.SHUTDOWN_ING
                || stage == TaskStage.STOPPED || stage == TaskStage.BREAKING) {
            MetricsHelper.counter(BifroTaskMetric.ILLEGAL_TASK_STATE,
                    Tags.of("stage", stage.name(), "taskId", taskConfig.getTaskId()));
            log.warn("MqttClient connect not allowed, clientId={}, taskStage={}", clientConfig.getClientId(), stage);
            return false;
        }
        if (status == CLOSED) {
