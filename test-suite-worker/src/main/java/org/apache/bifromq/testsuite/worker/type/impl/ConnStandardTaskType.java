...
import org.apache.bifromq.testsuite.worker.pipeline.stages.InitConnClientsStage;
...
public class ConnStandardTaskType implements TaskType {

    @Override
    public String typeId() {
        return "mqtt.conn.standard";
    }

    @Override
    public ExecutionPlan buildPlan(WorkerPlanSpec spec, Vertx vertx) {
        List<PipelineStage<TaskPipelineContext>> stages = List.of(
            new InitConnClientsStage(),
            new StartConnClientsStage(Constants.CONN_CLIENT_TAG, TaskEvent.ONGOING),
            new StressStage(vertx),
            new CleanupConnStage(Constants.CONN_CLIENT_TAG),
